# ☁️ Cloud Engineer Roadmap

Welcome to my Cloud & DevOps Engineering portfolio.

This repository documents my journey from beginner to Cloud Engineer while studying 2 hours every day.

---

# 👨‍💻 About Me

- 🌍 Ugandan living in Dubai
- 🎯 Goal: Become a Cloud & DevOps Engineer
- 📚 Currently studying Linux using Ubuntu and The Linux Command Line by William Shotts

---

# 🗺️ Learning Roadmap

## Phase 1 - Linux
- [ ] Ubuntu
- [ ] Linux Command Line
- [ ] Bash Scripting
- [ ] File Permissions
- [ ] Process Management

## Phase 2 - Git
- [ ] Git
- [ ] GitHub
- [ ] Git Workflow

## Phase 3 - Containers
- [ ] Docker
- [ ] Docker Compose

## Phase 4 - Cloud
- [ ] AWS Cloud Practitioner
- [ ] EC2
- [ ] IAM
- [ ] VPC
- [ ] S3

## Phase 5 - Infrastructure as Code
- [ ] Terraform

## Phase 6 - Container Orchestration
- [ ] Kubernetes

## Phase 7 - CI/CD
- [ ] GitHub Actions
- [ ] Jenkins

---

# 📚 Books

- [ ] The Linux Command Line — William Shotts
- [ ] (More books to come)

---

# 🏗️ Projects

| Project | Status |
|---------|--------|
| Linux Learning Journey | 🚧 In Progress |
| Bash Scripts | ⏳ Planned |
| Docker Labs | ⏳ Planned |
| AWS EC2 Lab | ⏳ Planned |
| Terraform Labs | ⏳ Planned |
| Kubernetes Labs | ⏳ Planned |

---

# 🏆 Certificates

Coming soon...

---

# 📅 Study Log

## Week 1

- Created GitHub portfolio
- Downloaded Ubuntu
- Installing Ubuntu on VirtualBox

---

## Goal

Become a professional Cloud & DevOps Engineer through consistent daily practice, real-world projects, and continuous learning.

> "Consistency beats intensity."
